...
pipeline {
    ...
    stages {
                        ...
                        packages: [
                            [$class: 'MavenPackage',
                                mavenAssetList: [
                                    [classifier: '',
                                    extension: 'jar',
                                    filePath: 'build/DevOpsUsach2020-1.0.0.jar'
                                ]
                            ],
                                mavenCoordinate: [
        ...
        stage("Paso 6: Descargar Nexus"){
            steps {
                script{
                    env.STAGE='Paso 6'
                    sh ' curl -X GET -u $NEXUS_USER:$NEXUS_PASSWORD "http://nexus:8081/repository/maven-usach-ceres/com/devopsusach2020/DevOpsUsach2020/1.0.0/DevOpsUsach2020-1.0.0.jar" -O'
                }
            }
        }
    ...
    post {
        success {
			slackSend color: 'good', message: "[Grupo6][Pipeline IC/CD][Rama: ${GIT_BRANCH}][Stage: ${env.STAGE}][Resultado:Éxito/Success]", teamDomain: 'devopsusach20-lzc3526', tokenCredentialId: 'token-slack', channel: "${CHANNEL}"
		}
		failure {
            slackSend color: 'danger', message: "[Grupo6][Pipeline IC/CD][Rama: ${GIT_BRANCH}][Stage: ${env.STAGE}][Resultado:Error/Fail]", teamDomain: 'devopsusach20-lzc3526', tokenCredentialId: 'token-slack', channel: "${CHANNEL}"
        }
    }
} 